...
public interface CpPaymentProcessingService {
     ...
     */
    ResponseEntity<GenericResponse<?>> processAuthorizeRequest(CPPaymentAuthorizationRequest request) throws JsonProcessingException;

    /**
     * This method takes the action for Capture operation only.
     * It shouldn't be called without passing the Capture Request.
     */
    ResponseEntity<GenericResponse<?>> processCaptureRequest(CPPaymentCaptureRequest request) throws JsonProcessingException;

    /**
     * This method takes the action for Card Void operation only.
     * It shouldn't be called without passing the Card Void Request.
     */
    ResponseEntity<GenericResponse<?>> processCardVoidRequest(CPPaymentCardVoidRequest request) throws JsonProcessingException;

    /**
     * This method takes the action for Refund operation only.
     * It shouldn't be called without passing the Refund Request.
     */
    ResponseEntity<GenericResponse<?>> processRefundRequest(CPPaymentRefundRequest request) throws JsonProcessingException;
}
